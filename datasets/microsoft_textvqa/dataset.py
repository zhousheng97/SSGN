...
class MICROTextVQADataset(TextVQADataset):
    def __init__(self, dataset_type, imdb_file_index, config, *args, **kwargs):
        super().__init__(
            dataset_type, imdb_file_index, config, *args, **kwargs
        )
        self._name = "microsoft_textvqa"
    ...
    def load_item(self, idx):    
        ...
        # Load edge feat
        current_sample['obj_ocr_edge_feat'] = torch.from_numpy(current_sample.image_info_2['obj_ocr_edge_feat']).float()  # [100, 50, 5]
        current_sample['ocr_obj_edge_feat'] = torch.from_numpy(current_sample.image_info_2['ocr_obj_edge_feat']).float()  # [50, 100, 5]
        current_sample['ocr_ocr_edge_feat'] = torch.from_numpy(current_sample.image_info_2['ocr_ocr_edge_feat']).float()
        current_sample['obj_obj_edge_feat'] = torch.from_numpy(current_sample.image_info_2['obj_obj_edge_feat']).float()
    ...
    def add_sample_details(self, sample_info, sample):
        ...
        # sample.question = sample_info['question']
        ...
        '''Get FastText embeddings for object labels'''
        # obj_labels = sample_info["obj_label"]
        # obj_context = self.context_processor({"tokens": obj_labels})
        # sample.fasttext_obj_feat = obj_context["text"]
        ...
        sample.context = context["text"][:50,:]
        sample.context_tokens = context["tokens"][:50]
        sample.context_tokens_enc = enc_obj2bytes(context["tokens"][:50]) 
        sample.context_feature_0 = context["text"][:50,:]  
