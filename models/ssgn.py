...
@registry.register_model("ssgn")
class SSGN(BaseModel):
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        '''expr: top-100 object -> top-36 object'''
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_mmt_in = (
            ...
            self.linear_ocr_bbox_to_mmt_in(ocr_bbox.to(torch.float32))
        )
        )
...
class MRG_Graph(nn.Module):  # mimic the GRU of the GGNN
    ...
    def _build_common_layer(self, module_name, hidden_size, edge_dim=5):
        ...
        # transform_edge = nn.Linear(edge_dim, hidden_size)
    ...
    def _build_compute_graph(self, edge_feat, ques, input_mask, ques_mask, module_name):
        ...
        ques_guided_edge_attn = (
            getattr(self, module_name + '_edge_attn')(torch.tanh(ques_feature + edge_feat))).squeeze(
            -1)  # ques_feature/edge_feat:[16, 100, 50, 768]
    ...
    def forward(self, sample_list, fwd_results):
        ...
        new_t_feat = torch.bmm(v2t_attn.transpose(1, 2), v_feat)  # batch, 50, 768
        new_v_feat = torch.bmm(t2v_attn.transpose(1, 2), t_feat)  # batch, 100, 768

        v_feat_1 = self.qv_feat_layer_1(v_feat) + self.qv_feat_layer_2(new_v_feat) + self.qv_feat_layer_3(v2t_feat)
        t_feat_1 = self.qt_feat_layer_1(t_feat) + self.qt_feat_layer_2(new_t_feat) + self.qt_feat_layer_3(t2v_feat)


        # Self-modal Graph:
        v2v_attn, v2v_feat = self._build_compute_graph(v2v_edge, q_emb, v_mask, q_mask, module_name='qv')
        t2t_attn, t2t_feat = self._build_compute_graph(t2t_edge, q_emb, t_mask, q_mask, module_name='qt')
        v2v_mask = torch.bmm(v_mask.unsqueeze(-1), v_mask.unsqueeze(1))  # [N, 100, 100]
        t2t_mask = torch.bmm(t_mask.unsqueeze(-1), t_mask.unsqueeze(1))  # [N, 50, 50]
        v2v_attn = v2v_attn * v2v_mask
        t2t_attn = t2t_attn * t2t_mask

        new_v_feat_1 = torch.bmm(v2v_attn.transpose(1, 2), v_feat_1)  # batch, 100, 768
        new_t_feat_1 = torch.bmm(t2t_attn.transpose(1, 2), t_feat_1)  # batch, 50, 768

        v_feat = self.qv_feat_layer_1(v_feat_1) + self.qv_feat_layer_2(new_v_feat_1) + self.qv_feat_layer_3(v2v_feat)
        t_feat = self.qt_feat_layer_1(t_feat_1) + self.qt_feat_layer_2(new_t_feat_1) + self.qt_feat_layer_3(t2t_feat)
...
class PrevPredEmbeddings(nn.Module):
    ...
    def forward(self, ans_emb, ocr_emb, prev_inds):
        assert prev_inds.dim() == 2 and prev_inds.dtype == torch.long  #[16,12]
        assert ans_emb.dim() == 2  #[5000,768]
